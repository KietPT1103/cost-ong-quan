...
export default function CafePosPage() {
  ...
  const totalTablePages = Math.max(1, Math.ceil(sortedTables.length / TABLES_PER_PAGE));
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      console.error(error);
      alert("Lỗi khi lưu bill. Vui lòng thử lại.");
    } finally {
      setIsPaying(false);
    }
  };
      ...
      <html>
        ...
        <body>
          ...
          <table>
            <tr class="total">
              <td>Tổng cộng</td>
              <td style="text-align:right;">${formatCurrency(receiptData.total)} đ</td>
            </tr>
          </table>
  ...
  const handleSeedTables = async () => {
    ...
    const seedNames = Array.from({ length: 99 }, (_, i) =>
  ...
  }, [showReceipt, receiptData, cartItems.length, isPaying, handlePay, handleConfirmPrint]);
          ...
          <section className="flex h-full flex-col bg-slate-50">
            <div className="border-b bg-white px-4 py-3 space-y-2">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="text-sm font-semibold text-slate-700">Bàn</span>
                  <Input
                    list="table-options"
                    value={tableNumber}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div className="w-full max-w-md rounded-2xl bg-white shadow-lg">
          <div className="border-b px-4 py-3">
            <h3 className="text-lg font-semibold text-slate-900">Xem trước bill</h3>
          ...
          <div className="max-h-[70vh] overflow-y-auto px-4 py-3 space-y-3">
            <div className="text-sm text-slate-700">
              ...
              {receiptData.note && (
                <p className="text-xs text-slate-500">Ghi chú: {receiptData.note}</p>
              )}
            </div>
          ...
          <div className="flex items-center justify-end gap-2 border-t bg-slate-50 px-4 py-3">
            ...
            <Button onClick={handleConfirmPrint} className="bg-sky-600 hover:bg-sky-700">
